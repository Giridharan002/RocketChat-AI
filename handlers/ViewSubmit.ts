            ...
            if (interaction_data.view.state) {
                const completions_options =
                    interaction_data.view.state[
                        `${AppSetting.NAMESPACE}_ask_chatai`
                    ];
                ...
                if (!prompt) {
                    ...
                    return {
                        success: false,
                        message: "User's question is undefined or null.",
                    };
                }
                ...
                GenAiStackQueryRequest(app, http, read, prompt, user).then(
                    (result) => {
                        ...
                        if (!output_options) {
                            logger?.error(
                                "Output options are undefined or null."
                            );
                            return;
                        }

                        for (var output of output_options) {
                            const [output_mode, room_id, thread_id] =
                                output.split("#");
                            read.getRoomReader()
                                .getById(room_id)
                                .then((room) => {
                                    ...
                                    switch (output_mode) {
                                        ...
                                        default:
                                            logger?.warn(
                                                `Unsupported output mode: ${output_mode}`
                                            );
                                            break;
                                    }
                                });
